function saveResults(summaryTable, filename)
    ...
    if isempty(summaryTable)
        error('MATLAB:saveResults:EmptyTable', 'Input table is empty, cannot save results');
    end

    % Check if the path to the file is valid and if the directory exists
    [pathstr, ~, ~] = fileparts(filename);
    if ~isempty(pathstr) && ~exist(pathstr, 'dir')
        error('MATLAB:saveResults:InvalidDirectory', 'Directory does not exist: %s', pathstr);
    end
    ...
    try
        ...
        error('MATLAB:saveResults:WriteFail', 'Failed to write table to file %s: %s', filename, e.message);
    end
end